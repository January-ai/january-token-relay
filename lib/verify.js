...
function endUserIdFrom(headers) {
  const endUserId = (headers['january-end-user-id'] ?? '').trim()
  if (!endUserId) {
    throw new SessionError('Send a January-End-User-ID header naming the user this token is for.')
  }
  return endUserId
}
 ...
 * in the January-End-User-ID header.
 *
